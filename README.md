...
Another challenge that occurred during the process was the integration of another Wazuh agent on the Ubuntu virtual machine and it's showing on the Wazuh dashboard. The reason why the installed agent wasn't initially displayed on the Wazuh dashboard was due to the firewall settings. The inbound traffic coming from the Wazuh Agent (Ubuntu virtual machine) needs to be opened through both the TCP and UDP connections so that the Wazuh Manager can listen to the agent. To do this, another firewall rule for the Wazuh Manager needs to be added for this to occur. This change needs to be done through the cloud provider settings.  Additionally, if the Uncomplicated Firewall (UFW) settings are enabled in both the Wazuh Manager and Wazuh agent, this should either be disabled or if you choose to have it remain enabled, allow any inbound connections coming from the ports that the Wazuh Manager listens to the agents on. 
